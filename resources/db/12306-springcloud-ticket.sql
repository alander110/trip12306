...
(
    `id`          bigint(120) NOT NULL AUTO_INCREMENT COMMENT 'ID',
    `username`    varchar(256) DEFAULT NULL COMMENT '用户名',
    `order_sn`    varchar(64)  DEFAULT NULL COMMENT '订单号',
    `status`      tinyint(1) DEFAULT NULL COMMENT '状态 0：请求下单成功 1：列车与余票不足 2：购票请求失败',
    `create_time` datetime     DEFAULT NULL COMMENT '创建时间',
    `update_time` datetime     DEFAULT NULL COMMENT '修改时间',
    `del_flag`    tinyint(1) DEFAULT NULL COMMENT '删除标识',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1881317736219426817 DEFAULT CHARSET=utf8mb4 COMMENT='订单追踪表';
